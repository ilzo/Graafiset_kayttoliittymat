<?php
   ...
   if( !empty($_POST['name']) or !empty($_POST['desc']) or !empty($_POST['date'])):
	$data = array('name' => $_POST['name'], 'description' => $_POST['desc'], 'email' => $_POST['email'], 'phonenum' => $_POST['cell']);
	$date = $_POST['date'];
	$time = $_POST['time'];
...
$STH = $DBH->prepare("INSERT INTO calendar (eName, eDescription, pEmail, pPhone, eDate) values (:name, :description, :email, :phonenum, :sqldate)");
$STH->execute($data);
